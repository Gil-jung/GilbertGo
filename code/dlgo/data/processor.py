...
class GoDataProcessor:
    ...
    def load_go_data(self, data_type='train',  # As `data_type` you can choose either 'train' or 'test'.
        ...
        index.download_files()  # We download all games from KGS to our local data directory. If data is available, it won't be downloaded again.
        ...
        for zip_name in zip_names:
            ...
            if not os.path.isfile(self.data_dir + '/' + data_file_name):
                self.process_zip(zip_name, data_file_name, indices_by_zip_name[zip_name])  # The zip files are then processed individually.

        features_and_labels = self.consolidate_games(data_type, data)  # Features and labels from each zip are then aggregated and returned.
    ...
    def process_zip(self, zip_file_name, data_file_name, game_list):
        ...
        total_examples = self.num_total_examples(zip_file, game_list, name_list)  # Determine the total number of moves in all games in this zip file.
