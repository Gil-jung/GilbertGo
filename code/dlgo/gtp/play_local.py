...
class LocalGtpBot():
    ...
    def __init__(self, agent, termination=None, handicap=0, 
                 ...
                 our_color='b'):
        self.bot = TerminationAgent(agent, termination)  # We initialize a bot from an agent and a termination strategy.
        self.handicap = handicap
        self._stopped = False  # We play until the game is stopped by one of the players.
        self.game_state = GameState.new_game(19)
        self.sgf = SGFWriter(output_sgf)  # At the end we write the the game to the provided file in SGF format

        self.our_color = Player.black if our_color == 'b' else Player.white
        self.their_color = self.our_color.other

        cmd = self.opponent_cmd(opponent)  # Our opponent will either be GNU Go or Pachi.
        pipe = subprocess.PIPE
        self.gtp_stream = subprocess.Popen(
            cmd, stdin=pipe, stdout=pipe,  # We read and write GTP commands from the command line.
            bufsize=0
        )
