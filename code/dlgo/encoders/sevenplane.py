            ...
            for col in range(self.board_width):
                ...
                if go_string is None:
                    if game_state.does_move_violate_ko(game_state.next_player, Move.play(p)):
                        board_tensor[6][row][col] = 1  # Encoding moves prohibited by the ko rule
                else:
                    liberty_plane = min(3, go_string.num_liberties) - 1
                    liberty_plane += base_plane[go_string.color]
                    board_tensor[liberty_plane][row][col] = 1  # Encoding black and white stones with 1, 2 or more liberties.
        return board_tensor
