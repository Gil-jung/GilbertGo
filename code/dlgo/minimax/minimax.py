...
def best_result(game_state):
    ...
    for candidate_move in game_state.legal_moves():
        next_state = game_state.apply_move(candidate_move)      # See what the board would look like if we play this move.
        opponent_best_result = best_result(next_state)          # Find out our opponent's best move.
        our_result = reverse_game_result(opponent_best_result)  # Whatever our opponent wants, we want the opposite.
        if our_result.value > best_result_so_far.value:         # See if this result is better than the best we've seen so far.
