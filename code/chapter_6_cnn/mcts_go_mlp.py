...
class MCTSAgentDNN(nn.Module):
    def __init__(self, IMG_SIZE=9):
        super(MCTSAgentDNN, self).__init__()
        self.fc1 = nn.Linear(IMG_SIZE*IMG_SIZE, 200)
        self.fc2 = nn.Linear(200, 300)
        self.fc3 = nn.Linear(300, 200)
        self.fc4 = nn.Linear(200, board_size)
...
model = MCTSAgentDNN(IMG_SIZE).cuda()
print(model)
...
torch.save(model.state_dict(), current_path + "\\models\\MCTSAgentDNN.pt")

model_test = MCTSAgentDNN(IMG_SIZE).cuda()
model_test.load_state_dict(torch.load(current_path + "\\models\\MCTSAgentDNN.pt"))
model_test.eval()
x, y = next(iter(test_loader))
x = x.cuda().view(-1, IMG_SIZE*IMG_SIZE)
y_ = model_test(x)
